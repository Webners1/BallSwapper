import { Token } from '@core-sdk'

export const WETH = new Token(7001, '0x0A67e05a87b87f210277542267ABD87F9D29CB67', 18, 'WZETA', 'Wrapped ZETA')

export const EMMET = new Token(7001, '0xc03B1253770a34289f8Aa44b694b190dF9bb69b8', 18, 'EMMET', 'EMMET')

export const ZNT = new Token(7001, '0x17ff5320C6fE629730dEAC6Ff0FD795246cfdD06', 18, 'ZNT', 'ZNS Token')

export const GETH = new Token(7001, '0x13A0c5930C028511Dc02665E7285134B6d11A5f4', 18, 'gETH', 'gETH')

// export const FRAX = new Token(ChainId.MOONBEAM, '', 18, 'FRAX', 'Frax')
// export const MIM = new Token(
//   ChainId.MOONBEAM,
//   '',
//   18,
//   'MIM',
//   'Magic Internet Money'
// )
